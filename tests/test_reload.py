...
from symphony.reload import EffectiveConfig, ReloadOutcome, WorkflowConfigStore
...
def _store(tmp_path: Path, **kwargs: object) -> WorkflowConfigStore:
    path = tmp_path / "WORKFLOW.md"
    _write(path)
    return WorkflowConfigStore(path, env={}, **kwargs)  # type: ignore[arg-type]
...
def test_initial_load_failure_propagates(tmp_path: Path) -> None:
    ...
    with pytest.raises(MissingWorkflowFileError):
        WorkflowConfigStore(tmp_path / "WORKFLOW.md", env={})
...
def test_relative_workspace_root_resolves_against_workflow_dir(tmp_path: Path) -> None:
    ...
    store = WorkflowConfigStore(path, env={})
    assert store.current.config.workspace.root == tmp_path / "ws"
...
def test_reload_reapplies_changed_config(tmp_path: Path) -> None:
    ...
    assert store.current.config.polling.interval_ms == 5000
...
def test_reload_picks_up_new_prompt_template(tmp_path: Path) -> None:
    ...
    store.reload()
    assert store.current.prompt_template == "NEW BODY"
...
def test_invalid_reload_keeps_last_known_good_and_emits_error(tmp_path: Path) -> None:
    ...
    store = WorkflowConfigStore(path, env={}, on_error=errors.append)
    ...
    assert store.current.config.polling.interval_ms == 30000
...
def test_reload_recovers_after_a_failed_reload(tmp_path: Path) -> None:
    ...
    _write(path, body="---\nnot: [closed\n")  # invalid YAML
    assert store.reload().applied is False

    _write(path, interval=1234)  # fixed again
    outcome = store.reload()
    assert outcome.applied is True
    assert store.current.config.polling.interval_ms == 1234
...
def test_poll_reloads_when_content_changes(tmp_path: Path) -> None:
    ...
    assert store.current.config.polling.interval_ms == 2500
    # Now stable again.
    assert store.poll() is None
...
def test_poll_detects_mtime_only_change(tmp_path: Path) -> None:
    ...
    outcome = store.poll()
    assert outcome is not None and outcome.applied is True
    assert store.current.config.polling.interval_ms == 20000
...
def test_poll_recovers_when_file_reappears(tmp_path: Path) -> None:
    ...
    path.unlink()
    assert store.poll().applied is False  # type: ignore[union-attr]

    _write(path, interval=4321)
    outcome = store.poll()
    assert outcome is not None and outcome.applied is True
    assert store.current.config.polling.interval_ms == 4321
